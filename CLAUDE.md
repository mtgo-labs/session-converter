...
- **Telethon** (`LonamiWebs/Telethon` v1 `sessions/string.py`): single version, prefix `"1"`, struct `>B{}sH256s` = `dc[1] + ip[4|16] + port[2 BE] + authKey[256]`, base64url *with* padding. `api_id` is never serialized. Telethon v2 (alpha, `v2` branch) has **no string-session format at all** (only `MemorySession`/`SqliteSession`), so `"1"` is the only real string-session prefix. The encoder emits exactly this; the decoder additionally tolerates a legacy `"2"`+`api_id` variant this library once emitted (a prior commit wrongly assumed a Telethon "v2 prefix 2" that doesn't exist).
